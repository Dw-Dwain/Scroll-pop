...
const SECTIONS = [
  ...
  { title: 'Data Storage & Security', body: "Your data is stored in PostgreSQL hosted by Fly.io in the Tokyo, Japan region, with row-level security policies enforcing tenant isolation. Event data flows through Cloudflare Workers and the Fly.io-hosted API, buffered in Upstash Redis (Tokyo region), before being persisted. All data in transit is encrypted via TLS. Sub-processors: Fly.io (API + database hosting, Tokyo), Cloudflare (edge delivery, R2, KV), Upstash (Redis), Clerk (authentication), Stripe (billing), PostHog (product analytics), Sentry (error monitoring), and Resend (transactional email). A Data Processing Agreement covering these sub-processors and international transfers (EU SCCs / UK Addendum / APPI) is available on request." },
