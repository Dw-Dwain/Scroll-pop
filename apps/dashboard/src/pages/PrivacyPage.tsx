...
const SECTIONS = [
  ...
  { title: 'Data Storage & Security', body: "All data is stored on managed PostgreSQL (Fly.io, Tokyo / Japan region) with row-level security policies enforcing tenant isolation. Event data flows through Cloudflare Workers and the Fly.io-hosted API, buffered in Upstash Redis (Tokyo), before being persisted — so first-party data resides in Japan. All data in transit is encrypted via TLS 1.2+. Sub-processors: Clerk (authentication), Stripe (billing), Cloudflare (edge delivery, R2, KV), Fly.io (API + database hosting), Upstash (Redis), Sentry (error monitoring), PostHog (product analytics), and Resend (transactional email). A Data Processing Agreement is available on request." },
