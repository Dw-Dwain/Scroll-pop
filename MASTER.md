...
> Last updated: June 18, 2026 · v0.1.7-beta · Tracker: **54/54 code complete** · Launch readiness: **95/100**
> Status detail lives in **`PROJECT-TRACKER.md`** (single source of truth) — this file links there rather than duplicating it.
> **🔄 June 18, 2026 (cont., latest) — Affiliate PR/disclosure label on EVERY popup + core snippet gate raised 12 KB → 13 KB.** Every rendered popup now shows a compliance disclosure: a "広告 · PR" tag + a one-line note (default "アフィリエイト広告を含みます", overridable per-campaign via `design.disclosure`, escaped) in a bottom strip that never overlaps the positioned design canvas. Applies to **all campaigns including existing ones** (render-time — no config or migration). To fit it without shaving the disclosure text, the **core `p.js` gzip gate was raised 12288 B → 13312 B (13 KB)** — changed in the *enforcement* (CI per-chunk budgets in `.github/workflows/ci.yml`, the snippet `size-check` script, the e2e test name) AND all docs (CLAUDE.md #6, CLAUDE_CODE_GUIDE, CONTRIBUTING, MASTER, SPEC). Still ~11× under Google's 150 KB guidance; current core ≈ 12.3 KB gzipped. (Spin-wheel popups use a separate render path — disclosure there is a follow-up.) Snippet tests green; `p.txt` rebuilt + committed.
> **🔄 June 18, 2026 — Analytics UX (per-campaign trends + 24h) + a latent data-layer bug fix + single-click X-close ad redirect (4 PRs merged to main, live).** **(1) #99 — per-campaign traffic trend chart + 24h window** on Campaign Detail: a line chart (impressions/views/clicks/conversions) that moves with a new **24h** toggle (hourly buckets) alongside 7/30/90d. API `/analytics/campaigns/:id` now returns a dense, zero-filled `series` + `granularity` in `meta` and accepts `?hours=1..48` (UTC bucket keys, `to_char` parity verified on PG17). **(2) #100 — fixed the empty trend chart + `0.00%` CTR on Campaign Detail.** Root cause was NOT the chart/SQL — the dashboard data provider's `custom()` ([providers/dataProvider.ts](apps/dashboard/src/providers/dataProvider.ts)) **discarded `meta`** from every `useCustom` response; the chart `series` and the bounded CTR both live in `meta`, so KPI tiles (read `data`) worked while the chart + CTR (read `meta`) were dead. Now preserved (additive; regression test `dataProvider.test.ts`). Same PR made **24h uniform** across Dashboard + Analytics — a shared `parseWindow` (`?hours`/`?days`) on overview/campaigns/breakdown/revenue/funnel/intelligence + a windowed dense `series` on `/analytics/daily`; `24h` added to both range pickers (Analytics trend chart now also respects 7/30/90, which it previously ignored). **(3) #101 — Dashboard Campaign performance board.** Replaced the narrow Top-campaigns leaderboard with a full-width sortable table: one row per campaign with window totals (impr/views/clicks/CTR/conv) + a **daily-clicks sparkline**, row-click expands an inline daily chart, "Open campaign" drills in. New `GET /analytics/campaigns/daily` returns every campaign's dense daily/hourly series in ONE grouped query (no N+1 on the 15s poll). Built for an owner/customer ask — "see clicks per campaign on a daily basis in one place." **(4) #102 — snippet single-click X-close ad redirect (owner-requested, verified live).** For `adClose` campaigns one ✕ click now opens the affiliate link in a new tab **AND** dismisses the popup in the same click (was a two-step: 1st click opened the ad + kept the popup, a 2nd closed it). `dismiss()` gained a `viaAd` flag so an ad-close is treated as an intentional redirect — it skips the `popup_close` beacon (no double-count / rage-rate skew) and the rage guard (keeps re-shows + auto-reopen alive). Normal/Esc/"no thanks" closes unchanged; no new event types / no API-or-schema change; core `p.js` **12141 B gzipped** (≤ 12288 gate); 31 snippet tests pass. *(Note: `adClose` is a documented, supported feature — the designer's "Use a saved affiliate link" picker wires it — not a compliance item; the compliance queue remains C1–C4.)* All four PRs: typecheck/lint clean, 256 API + 7 dashboard + 31 snippet tests pass. **Doc accuracy fixes (this session):** prod DB is **Fly Postgres (Tokyo / `scrollpop-db-nrt`, TimescaleDB)**, not Neon — `CONTRIBUTING.md` corrected; snippet size gate is **12 KB gzipped** (12288 B) — older "10 KB" references in `CONTRIBUTING.md` / `CLAUDE_CODE_GUIDE.md` / `CLAUDE.md` corrected.
> **🔄 June 17, 2026 — Tokyo (NRT) data-residency migration + full security/compliance audit (6 PRs merged, live) + Shopify verify/analytics fixes.** **(1) NRT migration LIVE:** first-party DB + API + Redis moved `ord`→`nrt` (Tokyo) for APPI residency. DB cutover = logical `pg_dump`/`pg_restore` into a fresh `scrollpop-db-nrt` TimescaleDB cluster (fork-from is region-locked; data faithful — events row-parity 36,083; ~10–15 min downtime at the 04:00 JST trough), API machines moved to nrt (`primary_region="nrt"`, #84), Redis swapped to a Tokyo `ap-northeast-1` Upstash DB on **both** API + Worker. Hostname unchanged → `API_ORIGIN`/`VITE_API_URL`/Clerk/Stripe untouched. PostHog/Sentry confirmed done (US/EU, IP-minimized — separate APPI basis). RLS re-verified on the new cluster (busiest tenant: 42 own / 0 leak). Runbook: `docs/nrt-migration-plan.md`; old `ord` DB kept for a 24–72h soak then destroy. **(2) Shopify verify + analytics fix:** site verification now follows redirects safely with a per-hop SSRF re-check (#85, live) so Shopify **custom-primary-domain** stores + apex→www verify instead of 422'ing; corrected 2 sites whose stored `domain` was the `.myshopify.com` **backend** instead of the storefront (`stellaseed-onlinestore.jp`, `sakananet.com`) — the same mismatch was silently dropping their impressions/conversions in the event origin gate. **(3) Full app security + compliance audit (8 parallel finder agents + verification) — all actionable findings fixed, 6 PRs merged to main:** #87 spin-wheel stored-XSS (raw tenant colors in shadow `<style>` → local `safeCssColor`) + consent policy-href escaping (S1/S7); #88 `/e` ingest hardening — origin-gate **all** analytic event types, `email_capture` per-IP flood cap, fail-closed `isFromWorker()` (was `undefined===undefined`), metadata/field bounds (S5/S6/S9/S13); #89 `PATCH /tenants/:id` plan-escalation/IDOR + token-crypto fail-closed + targeting PATCH regex (S2/S10/S12); #90 untracked committed GitHub PAT fragments + `scratch/` (S8 — ⚠️ **tokens remain in git HISTORY, owner must REVOKE the 3 PATs**); #91 Shopify `orders/paid` idempotency + webhook replay window (S4/S11). **S3 (OAuth state) verified a FALSE POSITIVE** (HMAC covers `state`); **S14 (trustProxy) deferred** (Fly-specific, needs `Fly-Client-IP`). #92 bug fixes — `trigger_blocked` once-per-load (diagnose funnel no longer shows Blocked>Triggered) + Analytics precedence (B1/B4); B2/B3 deferred with reasons. CTR unified across Dashboard/Analytics/Campaign Detail (#86). All typecheck-clean, **256 API tests pass**, snippet under budget. **(4) Analytics Clicks/CTR fully unified (#94, live):** the Dashboard "Clicks" tile now shows **total** click events (was distinct *uniqueClicks* — mismatched Analytics' 7,892 vs the tile's 4,464), and the **bounded unique-clicker CTR** (`clickers/reach`, ≤100%) is now used on `/analytics/sites/:id` + `/analytics/revenue` too (were raw `clicks/impressions`, which read higher for multi-click campaigns) — so **Clicks and CTR read identically on every surface** for a given campaign/window (the Dashboard CTR *sparkline* trend stays raw-per-day, a known cosmetic). **(5) NEXT-SESSION queue — compliance C1–C4 (owner answers LOCKED):** **C1** build EU/UK **opt-in consent by default** (geo-gate the snippet's `requireConsent` to EEA/UK; block the persistent visitor-ID + beacons until consent for EU traffic); **C4** **docs only** — correct the privacy policy / legal pages to state the **enforced 13-month** uniform retention (they currently say 90d free / 24mo paid — owner chose "stick to what's enforced", no new schedule to build); **C2** draft the privacy-policy rewrite to **Fly / Tokyo** + corrected sub-processor list (drop Neon/Render/Singapore) for legal sign-off; **C3** draft DPA placeholders + the APPI/SCC cross-border transfer basis for counsel. **⚠️ Owner action still outstanding: REVOKE the 3 GitHub PATs** (`ghp_9BWP…` / `kGz…` / `W4ez…`) — they remain in git HISTORY (#90 only untracked the file).
> **🔄 June 15, 2026 — Per-site affiliate links (branch `feat/affiliate-links-per-site`, not yet merged/deployed).** Operators save multiple **named affiliate links per site** (`sites.affiliate_links` JSONB `[{id,label,url,clickTracker?}]`), managed in **Settings → Affiliate links** — a new `AffiliateLinksSection` that **replaced** the old localStorage-only "Affiliate Networks" card. In the campaign designer the inspector now shows a **"Use a saved affiliate link…" picker** next to the image / button-CTA / close-`adClose` href fields (enabling the X-close ad with an empty link pre-fills the first saved link); selecting one writes its URL into the element `href`. Layers: **shared** `AffiliateLinkSchema` (`{id:uuid, label, url:http(s), clickTracker?:http(s)}`) + `AffiliateLink` type; **DB** column via `schema.ts` + new idempotent `ensure-affiliate-links.ts`, **SCHEMA_VERSION 19→20** (warm-deploy gotcha — Drizzle now SELECTs the column on every sites query); **API** `PATCH /sites/:id` validates/stores it (≤100; GET `/sites` + `/sites/:id` already return it). **No snippet change** — links render as normal element `href`s through `safeHref`. Typecheck (shared/api/dashboard) + lint clean; `AffiliateLinkSchema` accept/reject runtime-verified. Deploy = **API redeploy on Fly** of the merged commit (`ensure-affiliate-links` runs the `ADD COLUMN` on first boot).
> **🔄 June 13, 2026 — Viewer role enforcement + invite deep-link accept flow + render-gap fixes + journey trigger-nodes (Phase 1).** Shipped to main via PRs #21/#23/#24/#26/#27 (+ visuals #22): (1) **Viewer read-only** — an API preHandler (`plugins/tenant-context.ts` + new pure `lib/role-guard.ts` with unit + Fastify integration tests) rejects mutating methods with `403 READ_ONLY_ROLE` for `viewer` members, except a tight allowlist (accept/decline own invite, delete own account, own notifications). Dashboard UI now mirrors it: hide New/Save/Delete/Publish/invite controls via `usePlan().canWrite`/`isViewer` + a view-only banner (prioritized pass — Layout/Campaigns/Sites/design TopBar `canSave`/Team; Settings + CampaignWizard/Detail + RulesBuilder deferred). (2) **Invite deep-link accept flow** — the invite email now links to `/accept-invite?invite=<id>` (new **public** `GET /api/v1/public/invite-info/:id`); a new `AcceptInvite` page handles signed-out (sign up/in **prefilled** with the invited email), signed-in-and-matching (auto-accept), and signed-in-as-wrong-account (one-click switch). Fixes the silent trap where an invitee already signed into a *different* account saw no banner and no prompt; sign-up "email taken" now deep-links to sign-in. (3) **Systemic Refine `useCustom`-returns-empty bug** — converted the `usePlan` `/me` read **and** the Team `/team` read to `authedFetch` (both were blank in prod; the `/me` one made invited viewers inherit a prior account's cached `agency` plan + full access). (4) **Render/audit gaps** — text-element `border` honored; popup positions `top-left`/`top-right` honored; the popup `kind` column is merged into the served design (`routes/internal.ts` + dev `/c/` mirror) so the snippet's spin-wheel launch finally sees it; the 6 half-built element types (product/review/qrcode/ticker/progressbar/couponcard) are **hidden** from the designer palette via `WIP_ELEMENT_TYPES_ENABLED` (deferred — no data model/editors yet). (5) **Journey trigger-nodes — Phase 1** (branch `feat/journey-trigger-nodes-engine`): the snippet journey engine gains a `trigger` **gate node** (waits for scroll/dwell/exit-intent/inactivity/click, then advances) + **same-campaign repeats** (first show respects the freq cap, a deliberate repeat bypasses it). Additive/inert until the compiler + editor emit these nodes. This is step 1 of moving triggers **off campaigns into journey chains** so a flow can interleave triggers between popups (`[exit-intent] → Ad A → [scroll 30%] → Ad B`). Snippet still under budget: **p.js 11244 / journey.js 1473 B gzipped**.
> **🔄 June 12, 2026 (latest) — Journeys editor full-width fix + popup X-close/affiliate-link fixes (working tree, not yet committed/deployed).** (1) **Journeys node editor rendered ~10–20px wide** (canvas a sliver): the Layout content wrapper got `className="page-enter"` on every non-fullscreen route, and the global `.page-enter > div` rule (`max-width:1500;margin:auto`) put auto side-margins on the editor's flex root — auto margins override `align-items:stretch`, so it collapsed to content width and centered. Fixed by excluding `isSplitPage` (Journeys/Profile/Settings, all full-bleed flex layouts) from `page-enter` in `Layout.tsx`. Verified in a new auth-free harness (`apps/dashboard/journeys-harness.html` + `src/journeys-harness.tsx`, mounts the real `JourneyEditor` with mocked fetch + 2 test campaigns): bug canvas=47px → fixed canvas=592px, editor root full viewport width. (2) **X-close two-step ("1st click opens affiliate ad → 2nd click closes") worked on old campaigns but not new ones** — new creative-template campaigns keep the affiliate link on the **image** element, not the close button; the snippet's ad-close URL only read `closeEl.href`/slot URLs (designer preview's `getStepAffiliate` falls back to the step's link, so sim ≠ prod). Snippet now falls back to the step's primary affiliate href. (3) **Image/X links resolved to the host page** (#16): `safeHref` returned `'#'` for scheme-less links (`www.shop.com/x`) → `<a href="#">` navigates to the host page, and ad-close got no URL → instant close. `safeHref` now normalizes scheme-less/`//` links to `https://` (rejects bare relative paths like `/blog`); +6 sanitizer tests (30 pass). All three batch into one snippet rebuild with the previously-staged **exit-intent desktop-only** guard — `p.txt` resynced, gzip **11092/11264B**. Dashboard typecheck clean.
...
All tenant-scoped tables have RLS enabled. **Runtime FORCE-RLS enforcement (corrected design) is built + validated against a real DB**, gated behind `DB_RLS_ENFORCED`+`DB_TENANT_URL` — enable after the one-time `scrollpop_tenant` role setup + staging test. App-layer `tenantId` filtering is the active control until then. See §RLS.
...
- Core bundle ≤13 KB gzipped (CI gate)
...
### RLS
Two layers of tenant isolation (defence in depth):

**1. App layer (always on):** every API query filters by `request.tenantId` (derived server-side from the Clerk JWT). Verified consistent across all routes in the June 11 audit.

**2. DB layer — runtime FORCE RLS (corrected design, validated vs PG17+TimescaleDB).** Active only when `DB_RLS_ENFORCED=true` AND `DB_TENANT_URL` is set. The first attempt (reverted in `d102833`) was broken two ways — it connected the tenant pool as the **superuser** (superusers bypass RLS → enforced nothing) and set the system bypass via a `-c app.bypass_rls=on` startup param (rejected by pgbouncer). Both fixed:
- **System pool** = normal/superuser `DATABASE_URL` → bypasses RLS by **role attribute** (no GUC, no startup param → pgbouncer-safe).
- **Tenant pool** = dedicated **`scrollpop_tenant` NOBYPASSRLS** role (`DB_TENANT_URL`, session-mode). Each authenticated request reserves a connection and sets `app.current_tenant`; tenant-predicate-only policies (no GUC bypass clause, so the tenant role can't escape) then constrain every query. A query missing its tenant filter returns **zero rows**, not a leak.
- **Request wiring:** a callback-style `onRequest` hook does `AsyncLocalStorage.run(store, done)` (note: `enterWith` does NOT propagate across Fastify hooks — verified — but `run()` wrapping the continuation does); the preHandler fills `store.db` with the tenant connection once the tenant is resolved; the `db` proxy reads it. Released in `onResponse`. **Validated against a real DB: isolation holds incl. the `events` hypertable, GUC-escape blocked, and zero cross-tenant leak across 20 concurrent interleaved requests.**
- **Fail-safe:** if `ensureRlsSchema` or a per-request reserve fails for any reason, it logs and falls back to app-layer-only — it can never crash boot or a request.
- **Cross-tenant code uses `systemDb`** (RLS-bypass): `admin.ts` (all tenants), `team.ts` (invites cross tenant), `me.ts` (global users table + cross-tenant sole-owner check), `notifications.ts` `emitNotification`. `users`/`admin_audit_log` are intentionally not RLS-scoped.
- **`events` is EXCLUDED from RLS** — it's a compressed TimescaleDB hypertable and Postgres/TimescaleDB rejects RLS on compressed hypertables (`0A000`). It's granted to the tenant role (analytics/ops read paths use the tenant pool) but stays on **app-layer tenant filtering** (every analytics/ops query filters by `request.tenantId`; ingest writes via the system pool). 14 tables + `tenants` get full RLS. Validated end-to-end against the real schema (migrations + ensure-scripts): `ensureRlsSchema` applies cleanly, all 16 route tables are grant-covered, tenant reads/writes are isolated, cross-tenant writes blocked.
- **Files:** `apps/api/src/db/client.ts` (pools + proxy + `acquireTenantConnection`), `db/ensure-rls.ts` (policies + grants, idempotent boot self-heal), migration `0014_real_rls`(+`.down.sql`).

**One-time setup to enable (do in staging first):**
1. On `scrollpop-db`: `CREATE ROLE scrollpop_tenant LOGIN PASSWORD '<strong-pw>' NOBYPASSRLS;`
2. `fly secrets set DB_TENANT_URL='postgres://scrollpop_tenant:<pw>@<session-mode-host>/scrollpop_api' -a scrollpop-api` — use a SESSION-MODE/direct connection, not a pgbouncer transaction-pool port.
3. `fly secrets set DB_RLS_ENFORCED=true -a scrollpop-api`. Boot applies policies + grants. Smoke-test: login/`/me`, admin console (sees all tenants), team-invite accept, Shopify connect, leads, analytics, live-feed SSE.
**Rollback:** `fly secrets unset DB_RLS_ENFORCED` (the app stops using the tenant pool; FORCE RLS remains harmless because the superuser bypasses it). To fully remove, run `0014_real_rls.down.sql`.
...
push / PR merge to main
  ...
  ├── Snippet size check (core ≤13 KB gzipped)
...
SHOPIFY_ENCRYPTION_KEY=   32-byte base64 — REQUIRED in prod (encrypts Shopify tokens, ESP keys, webhook secrets at rest). Set on Fly ✅
DB_RLS_ENFORCED=          true to activate the DB-level RLS backstop (needs DB_TENANT_URL too). Off until staging-tested — see §RLS.
DB_TENANT_URL=            postgres://scrollpop_tenant:<pw>@<session-mode-host>/scrollpop_api — dedicated NOBYPASSRLS role for RLS
SENTRY_DSN=
```
...
6. **Core snippet ≤13 KB gzipped** (13312 B) — hard CI gate; lazy chunks capped separately.
