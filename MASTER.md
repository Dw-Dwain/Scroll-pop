...
> **June 11:** **Adversarial security re-review + full Top-20 remediation (commit `72d606f`).** Fresh penetration-style audit (auth, multi-tenant, API, infra, snippet, privacy). Verdict: app-layer tenant filtering already consistent on every route; no SQLi/XSS/mass-assignment/IDOR; webhooks timing-safe. Fixed 19 of 20 priorities — API builds clean, **82 vitest pass** (incl. new `src/security-fixes.test.ts`). **C-1 (DB-level RLS backstop) was attempted, then REVERTED (`d102833`):** the two-pool + per-request `app.current_tenant` GUC design **crash-looped on Fly's pooled connection** — pgbouncer (in front of postgres-flex) rejects the `options` startup parameter used for the bypass GUC and drops session GUCs in transaction-pooling mode, so `ensureRlsSchema` couldn't get a usable connection and boot threw → restart loop. The runtime wiring is now inert (`rlsEnforced=false` constant; `DB_RLS_ENFORCED` can no longer crash the app). **C-1 deferred:** doing it right needs a SESSION-MODE/direct connection (`DIRECT_DATABASE_URL`, not the pgbouncer pooler) for the tenant pool + a `BYPASSRLS`/`NOBYPASSRLS` DB-role pair on `scrollpop-db` — provision + test before re-attempting. Migration `0014_real_rls` + `ensure-rls.ts` retained as artifacts. **Tenant isolation stays enforced at the app layer (every query filters by `request.tenantId` — verified consistent in this audit).** Other fixes (all live): **H-1** admin check fails CLOSED on Clerk error (5-min verified-cache, no DB-email fallback); **H-2** Clerk webhook stores the verified PRIMARY email; **H-3** site-verification fetches use a shared SSRF guard (`lib/url-guard.ts`) + `redirect:error` + no reflected key; **M-1/M-2** ESP keys + outbound-webhook secrets encrypted at rest (AES-256-GCM); **M-3** token-crypto refuses plaintext in prod (now requires `SHOPIFY_ENCRYPTION_KEY`, **set on Fly ✅**); **M-4** dev `/c`+`/v1` routes 404 in prod; **M-5** dropped `*`+credentials on `/e`; **L-1** dashboard CSP now enforcing; **L-2** ops `campaignId` UUID-validated; **L-3** `/e` revenue honored only from the Worker; **L-4** no emails in admin logs. Still ops-only: set Stripe secrets on Fly; rotate any pre-June-11 ESP/webhook secrets (re-save in dashboard re-encrypts).
...
All tenant-scoped tables have RLS enabled (permissive policies). **Runtime FORCE-RLS enforcement is DEFERRED** — a June 11 attempt crash-looped on Fly's pgbouncer pooler and was reverted; app-layer `tenantId` filtering is the active control. See §RLS.
...
### RLS
Tenant isolation:
1. **App layer (ACTIVE — the control today):** every API query filters by `request.tenantId` (derived server-side from the Clerk JWT). Verified consistent across all routes in the June 11 audit.
2. **DB layer (FORCE RLS) — DEFERRED.** A runtime enforcement attempt (June 11) crash-looped on Fly's pgbouncer pooler (rejects the `options` startup param; drops session GUCs in transaction-pool mode) and was reverted (`d102833`). The runtime wiring is inert (`rlsEnforced=false`). Re-attempt requires: a **session-mode/direct connection** (`DIRECT_DATABASE_URL`, bypassing pgbouncer) for the per-request tenant pool, AND a `BYPASSRLS` role (system paths) + `NOBYPASSRLS` role (tenant pool) on `scrollpop-db`. Artifacts kept: migration `0014_real_rls` (+`.down.sql`), `apps/api/src/db/ensure-rls.ts`, and the two-pool/proxy design (in git history at `72d606f`). When re-attempting, smoke-test auth, admin console, team-invite accept, Shopify connect, leads, analytics against the direct connection in staging first.
...
DB_RLS_ENFORCED=          (DEFERRED — inert no-op; crash-looped on pgbouncer June 11. Needs DIRECT_DATABASE_URL + RLS roles before re-enabling. Do NOT set true on the current pooled connection.)
SENTRY_DSN=
```
