...
export default function SidebarLeft({
  ...
  // Fully covers all 31 requested categories in organized filter chips
  const categories = [
    'All',
    'USA Holidays',
    'Japan Holidays',
  ...
  return (
    <div className="flex h-full w-[400px] shrink-0 border-r border-zinc-200 bg-white select-none">
      {/* 1. Left Icon Navigation Rail */}
      <div className="flex w-[80px] flex-col items-center border-r border-zinc-200 bg-zinc-50 py-4 gap-4 justify-between shrink-0">
        <div className="flex flex-col items-center gap-3 w-full">
          ...
          >
            ...
            {elements.length > 0 && (
              <span className="absolute top-1 right-1 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-zinc-900 border border-white text-[8px] font-bold text-white shadow-xs">
                {elements.length}
              </span>
            )}
          </button>
        ...
        <div className="text-[8px] tracking-widest text-zinc-400 font-mono scale-90 pt-4 border-t border-zinc-200 w-full text-center select-none uppercase">
          c-pop
        </div>
      </div>

      {/* 2. Right Expanded Navigation Drawer */}
      <div className="flex flex-1 flex-col overflow-hidden bg-white p-5">
        
        {/* TEMPLATES DRAWER */}
        {activeTab === 'templates' && (
          <div className="flex h-full flex-col overflow-hidden text-left">
            <div className="mb-4">
              <h3 className="text-xs font-semibold text-zinc-900 tracking-tight uppercase font-mono flex items-center gap-1.5">
                <Sparkles className="h-3.5 w-3.5 text-zinc-800" />
                Premium Presets Catalog
              </h3>
              <p className="text-[11px] text-zinc-500 mt-1 leading-normal">Select an expert layout crafted specifically to maximize customer list conversions.</p>
            </div>

            {/* Category selection */}
            <div className="flex gap-1 overflow-x-auto pb-2 scrollbar-none shrink-0 border-b border-zinc-200 mb-4 select-none">
              {categories.map(cat => (
                <button
                  key={cat}
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1 scrollbar-thin">
              <div className="text-[9px] font-semibold text-zinc-400 tracking-wider uppercase font-mono mb-1">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1 pb-4 scrollbar-thin select-none">
              ...
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    { id: 'emerald', name: 'Emerald Gold', bg: 'bg-emerald-950 text-amber-500', color: '#F59E0B' },
                    { id: 'alarm', name: 'Flash Coral', bg: 'bg-red-50 text-red-650', color: '#EF4444' }
                  ].map(style => (
                    <button
                      key={style.id}
                      ...
                      className={`flex items-center gap-2 p-2 border rounded-lg transition-all duration-150 cursor-pointer text-left ${
                        mixerStyle === style.id
                          ...
                          : 'border-zinc-200 bg-white hover:border-zinc-450'
                      }`}
                    >
                      <span className="h-2.5 w-2.5 rounded-full shrink-0" style={{ backgroundColor: style.color }} />
                      <span className="text-[11px] font-semibold text-zinc-800">{style.name}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full text-xs font-semibold text-zinc-805 bg-white p-2.5 border border-zinc-200 focus:border-zinc-900 rounded-lg outline-hidden focus:ring-1 focus:ring-zinc-900"
                >
        ...
        {activeTab === 'elements' && (
          <div className="flex h-full flex-col text-left">
            <div className="mb-4">
              <h3 className="text-xs font-semibold text-zinc-900 tracking-tight uppercase font-mono">Campaign Elements</h3>
              <p className="text-[11px] text-zinc-500 mt-1 leading-normal">Add custom interactive components. Click any block below to inject directly onto your canvas step layout.</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1 scrollbar-thin">
              <div className="grid grid-cols-2 gap-2.5">
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center p-3 border border-zinc-200 rounded-md bg-zinc-50/50 hover:bg-zinc-100 hover:border-zinc-900 transition-all duration-150 text-zinc-800 cursor-pointer text-center group"
                >
                  <Type className="h-5 w-5 text-zinc-700 mb-1.5 group-hover:scale-105 transition-transform" />
                  <span className="text-xs font-semibold">Large Title</span>
                  <span className="text-[9px] text-zinc-400 mt-0.5">Headline block</span>
                </button>
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center p-3 border border-zinc-200 rounded-md bg-zinc-50/50 hover:bg-zinc-100 hover:border-zinc-900 transition-all duration-150 text-zinc-800 cursor-pointer text-center group"
                >
                  <Plus className="h-5 w-5 text-zinc-700 mb-1.5 group-hover:scale-105 transition-transform" />
                  <span className="text-xs font-semibold">Paragraph</span>
                  <span className="text-[9px] text-zinc-400 mt-0.5">Captions & body</span>
                </button>
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center p-3 border border-zinc-200 rounded-md bg-zinc-50/50 hover:bg-zinc-100 hover:border-zinc-900 transition-all duration-150 text-zinc-800 cursor-pointer text-center group"
                >
                  <Zap className="h-5 w-5 text-zinc-700 mb-1.5 group-hover:scale-105 transition-transform" />
                  <span className="text-xs font-semibold">Action Button</span>
                  <span className="text-[9px] text-zinc-400 mt-0.5">CTA & click promo</span>
                </button>
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center p-3 border border-zinc-200 rounded-md bg-zinc-50/50 hover:bg-zinc-100 hover:border-zinc-900 transition-all duration-150 text-zinc-800 cursor-pointer text-center group"
                >
                  <FormInput className="h-5 w-5 text-zinc-700 mb-1.5 group-hover:scale-105 transition-transform" />
                  <span className="text-xs font-semibold">Email Input</span>
                  <span className="text-[9px] text-zinc-400 mt-0.5">Collect addresses</span>
                </button>
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center p-3 border border-zinc-200 rounded-md bg-zinc-50/50 hover:bg-zinc-100 hover:border-zinc-900 transition-all duration-150 text-zinc-800 cursor-pointer text-center group"
                >
                  <Clock className="h-5 w-5 text-zinc-700 mb-1.5 group-hover:scale-105 transition-transform" />
                  <span className="text-xs font-semibold">Countdown</span>
                  <span className="text-[9px] text-zinc-400 mt-0.5">Limited offer clock</span>
                </button>
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center p-3 border border-zinc-200 rounded-md bg-zinc-50/50 hover:bg-zinc-100 hover:border-zinc-900 transition-all duration-150 text-zinc-800 cursor-pointer text-center group"
                >
                  <FolderSync className="h-5 w-5 text-zinc-700 mb-1.5 group-hover:scale-105 transition-transform" />
                  <span className="text-xs font-semibold">Product Card</span>
                  <span className="text-[9px] text-zinc-400 mt-0.5">Featured item node</span>
                </button>
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center p-3 border border-zinc-200 rounded-md bg-zinc-50/50 hover:bg-zinc-100 hover:border-zinc-900 transition-all duration-150 text-zinc-800 cursor-pointer text-center group"
                >
                  <Star className="h-5 w-5 text-zinc-700 mb-1.5 group-hover:scale-105 transition-transform" />
                  <span className="text-xs font-semibold">Reviews</span>
                  <span className="text-[9px] text-zinc-400 mt-0.5">Proof star reviewer</span>
                </button>
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center p-3 border border-zinc-200 rounded-md bg-zinc-50/50 hover:bg-zinc-100 hover:border-zinc-900 transition-all duration-150 text-zinc-800 cursor-pointer text-center group"
                >
                  <QrCode className="h-5 w-5 text-zinc-700 mb-1.5 group-hover:scale-105 transition-transform" />
                  <span className="text-xs font-semibold">QR Code</span>
                  <span className="text-[9px] text-zinc-400 mt-0.5">Offline mobile scan</span>
                </button>
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center p-3 border border-zinc-200 rounded-md bg-zinc-50/50 hover:bg-zinc-100 hover:border-zinc-900 transition-all duration-150 text-zinc-800 cursor-pointer text-center group"
                >
                  <ShieldAlert className="h-5 w-5 text-zinc-700 mb-1.5 group-hover:scale-105 transition-transform" />
                  <span className="text-xs font-semibold">Demand Tag</span>
                  <span className="text-[9px] text-zinc-400 mt-0.5">Urgent low stock text</span>
                </button>
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center p-3 border border-zinc-200 rounded-md bg-zinc-50/50 hover:bg-zinc-100 hover:border-zinc-900 transition-all duration-150 text-zinc-800 cursor-pointer text-center group"
                >
                  <Image className="h-5 w-5 text-zinc-700 mb-1.5 group-hover:scale-105 transition-transform" />
                  <span className="text-xs font-semibold">Image Asset</span>
                  <span className="text-[9px] text-zinc-400 mt-0.5">Brand graphic node</span>
                </button>
              </div>

              {/* Decorative instructions */}
              <div className="p-4 bg-zinc-50 border border-zinc-200 rounded-lg text-center mt-4">
                <span className="text-[10px] text-zinc-650 font-medium font-mono uppercase tracking-wider block mb-1">DESIGN TIP</span>
                <p className="text-[11px] text-zinc-500 leading-relaxed font-sans">
                  Introduce elements dynamically onto the layout container, drag them across precise coordinates, and adjust colors inside the inspector rails.
                </p>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'triggers' && (
          <div className="flex h-full flex-col overflow-hidden text-left">
            <div className="mb-4">
              <h3 className="text-xs font-semibold text-zinc-900 tracking-tight uppercase font-mono">Campaign Triggers</h3>
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1 pb-4 scrollbar-thin">
              {/* 1. Exit intent */}
              <div className="p-3.5 border border-zinc-200 rounded-lg bg-zinc-50/55 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Zap className="h-4 w-4 text-zinc-850" />
                    <span className="text-xs font-semibold text-zinc-900">Exit Intent System</span>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    />
                    <div className="w-8 h-4.5 bg-zinc-200 peer-focus:outline-hidden rounded-full peer peer-checked:after:translate-x-3.5 peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-zinc-300 after:border after:rounded-full after:h-3.5 after:w-3.5 after:transition-all peer-checked:bg-zinc-900"></div>
                  </label>
                </div>
              ...
              {/* 2. Scroll Percentage */}
              <div className="p-3.5 border border-zinc-200 rounded-lg bg-zinc-50/55 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <ArrowDown className="h-4 w-4 text-zinc-855" />
                    <span className="text-xs font-semibold text-zinc-900">Scroll Depth Trigger</span>
                  </div>
                  <span className="text-[11px] font-mono font-medium text-zinc-900 bg-zinc-100 px-2 py-0.5 rounded">
                ...
                <input
                  ...
                  className="w-full accent-zinc-900 cursor-pointer h-1 bg-zinc-200 rounded-lg hover:accent-black"
                />
                <p className="text-[10.5px] text-zinc-500 leading-normal font-sans">
                  Loads after shopper scrolls down a target depth coefficient of the organic skincare collection.
                </p>
              </div>
              ...
              <div className="p-3.5 border border-zinc-200 rounded-lg bg-zinc-50/55 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Clock className="h-4 w-4 text-zinc-855" />
                    <span className="text-xs font-semibold text-zinc-900">Idle Inactivity</span>
                  </div>
                  <span className="text-[11px] font-mono font-medium text-zinc-900 bg-zinc-100 px-2 py-0.5 rounded">
                ...
                />
                <p className="text-[10.5px] text-zinc-500 leading-normal font-sans">
                  Launches when the shopper remains static with no interactions or moves for some duration.
                </p>
              </div>
              ...
              <div className="p-3.5 border border-zinc-200 rounded-lg bg-zinc-50/55 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Calendar className="h-4 w-4 text-zinc-855" />
                    <span className="text-xs font-semibold text-zinc-900">Landing Time Delay</span>
                  </div>
                  <span className="text-[11px] font-mono font-medium text-zinc-900 bg-zinc-100 px-2 py-0.5 rounded">
              ...
              <div className="p-3.5 border border-zinc-200 rounded-lg bg-zinc-50/55 space-y-3">
                <div className="flex items-center gap-2">
                  <Smartphone className="h-4 w-4 text-zinc-855" />
                  <span className="text-xs font-semibold text-zinc-900">Device Restrictions</span>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {['all', 'desktop', 'mobile'].map((d) => (
                    <button
                      ...
                      className={`py-1.5 px-2 rounded text-[11px] font-medium border capitalize transition-all duration-150 cursor-pointer text-center ${
                        campaign.triggers.deviceTargeting === d
                          ? 'bg-zinc-900 border-zinc-900 text-white shadow-xs'
                          : 'bg-white border-zinc-200 text-zinc-650 hover:bg-zinc-100 hover:text-zinc-900 hover:border-zinc-350'
                      }`}
                    >
                      {d}
                    </button>
              ...
              <div className="p-3.5 border border-zinc-200 rounded-lg bg-zinc-50/55 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-zinc-900">Geo Targeting Regions</span>
                </div>
                <select
                  value={campaign.triggers.geoTargeting}
                  onChange={(e) => onUpdateTriggers('geoTargeting', e.target.value)}
                  className="w-full p-2 border border-zinc-200 rounded-md text-xs bg-white focus:ring-1 focus:ring-zinc-900 focus:border-zinc-900 outline-hidden font-medium text-zinc-850"
                >
        ...
        {activeTab === 'brands' && (
          <div className="flex h-full flex-col text-left font-sans">
            <div className="mb-4">
              <h3 className="text-xs font-semibold text-zinc-900 tracking-tight uppercase font-mono">Brand Architect</h3>
              ...
              <div className="space-y-2.5">
                {BRAND_PALETTES.map((brand) => (
                  <div
                    ...
                    className="p-3 border border-zinc-200 rounded-lg hover:border-zinc-900 cursor-pointer bg-white group flex flex-col gap-2 transition-all duration-150"
                  >
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-semibold text-zinc-900 group-hover:text-zinc-950 transition-colors">
                        {brand.name}
                      </span>
                      <span className="text-[9px] bg-zinc-100 px-1.5 py-0.5 rounded font-mono font-medium text-zinc-500 uppercase tracking-widest">
                        {brand.fontHeading}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="h-3.5 w-3.5 border border-black/10 rounded-full shrink-0" style={{ backgroundColor: brand.backgroundColor }} title="Canvas BG" />
                      <span className="h-3.5 w-3.5 border border-black/10 rounded-full shrink-0" style={{ backgroundColor: brand.primaryColor }} title="Primary / Heading Color" />
                      <span className="h-3.5 w-3.5 border border-black/10 rounded-full shrink-0" style={{ backgroundColor: brand.secondaryColor }} title="Secondary Highlight" />
                      <span className="h-3.5 w-3.5 border border-black/10 rounded-full shrink-0" style={{ backgroundColor: brand.textColor }} title="Paragraph text color" />
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'layers' && (
          <div className="flex h-full flex-col text-left font-sans">
            <div className="mb-4">
              <h3 className="text-xs font-semibold text-zinc-900 tracking-tight uppercase font-mono">Canvas Layers ({elements.length})</h3>
              <p className="text-[11px] text-zinc-500 mt-1 leading-normal">Control stacking hierarchy, rename elements, toggle visibility filters, or drag indices.</p>
            </div>

            <div className="flex-1 overflow-y-auto pr-1 scrollbar-thin">
              {elements.length === 0 ? (
                <div className="h-44 flex flex-col items-center justify-center p-4 border border-dashed border-zinc-200 rounded-lg bg-zinc-50/50">
                  <Layers className="h-6 w-6 text-zinc-300 mb-2" />
                  <span className="text-[11px] text-zinc-400 font-medium text-center leading-normal">Empty workspace canvas. Click the Elements tab above to insert active element nodes.</span>
                </div>
              ) : (
