...
const STORE_PRODUCTS: Product[] = [
  ...
  },
  {
    ...
    rating: 4.7,
    category: 'Masks',
    image: 'https://images.unsplash.com/photo-1590156546746-c2370ae25d71?auto=format&fit=crop&w=300&q=80',
  },
  {
    id: 'p4',
  ...
  return (
    <div className="fixed inset-0 bg-zinc-950/70 backdrop-blur-xs z-50 flex flex-col md:flex-row items-center justify-center p-3 md:p-6 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col bg-white rounded-b-2xl md:rounded-r-2xl border border-slate-200 h-[480px] md:h-[650px] overflow-hidden relative">
        ...
        {showMainCampaign && (
          <div className="absolute inset-0 bg-black/60 z-[990] flex items-center justify-center p-4">
            ...
            {(() => {
              ...
              const isWheel = activeStepConfig.elements.some(e => e.content === 'wheel');

              return (
                <div 
                  className="relative shadow-2xl transition-all duration-300 relative text-left"
                  style={{
                    width: `${activeStepConfig.width}px`,
                    height: `${activeStepConfig.height}px`,
                    backgroundColor: activeStepConfig.backgroundColor,
                    borderRadius: `${activeStepConfig.borderRadius}px`,
                    borderWidth: `${activeStepConfig.borderWidth}px`,
                    borderColor: activeStepConfig.borderColor,
                    boxShadow: activeStepConfig.boxShadow || '0 25px 80px rgba(0,0,0,0.25)',
                  }}
                >
                  ...
                  <button 
                    onClick={() => {
                      ...
                      setShowTeaser(true);
                      showToast('❌ Popup Campaign Minimised to Teaser Trigger');
                    }}
                    className="absolute top-3.5 right-3.5 h-6 w-6 rounded-full border border-zinc-200 hover:bg-zinc-100 flex items-center justify-center text-zinc-900 font-mono text-xs select-none cursor-pointer z-[1000] bg-white transition-all shadow-xs"
                  >
                    ✕
                  </button>
                  ...
                  {activeStepConfig.elements.map((el) => {
                    return (
                      ...
                      >
                          ...
                          <button 
                            onClick={() => {
                              if (campaignStep === 'main') {
                                ...
                                // Close modal and copy code automatically!
                        ...
                        {/* Close campaign triggers directly */}
                        {el.type === 'close' && (
                          <div 
                            onClick={() => {
                              setShowMainCampaign(false);
                            ...
                            className="w-full h-full bg-white text-gray-800 flex items-center justify-center border hover:bg-gray-100 transition-all cursor-pointer font-extrabold text-[10px] rounded-full shadow-sm"
                          >
                            ✕
                          </div>
                        )}
