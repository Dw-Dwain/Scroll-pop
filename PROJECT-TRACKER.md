...
| NQ-6 | 🔄 **diagnose fixed; runtime builder still TODO** | Journeys runtime (→ FU-7) | **Correction (Jun 11): the "fabricated 40/35/25%" claim is stale** — `routes/journeys.ts` `/journeys/:id/diagnose` already computes `topBlockedReasons` from **real** `trigger_blocked` events (`metadata.reason`); empty = nothing blocked, not faked. The Journeys **page** is currently an honest read-only per-campaign diagnostic. **Still open (the big one):** turn it into a real **node/tree journey runtime builder** — same-domain campaign chaining, advance-on-dismiss, max-2 guardrails, delay/cooldown, timers/triggers/branches/next-step — and move chaining out of the campaign editor. This is multi-session work (schema + migration + snippet chaining under the 10 KB budget + flow-graph UI + Playwright). Flag `ff_journeys_ui` (off). |
