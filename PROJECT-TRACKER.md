...
| NQ-2 | 🔄 **partly done (Jun 11)** | Experiments honesty + visual A/B | The Experiments **page** is already honest: it reads the **real variants** system (`/variants` + `/variants/results`) with real telemetry, not the unwired `experimentsV1` allocation. **Jun 11: added real visual A/B preview cards** (variant A vs B render their actual saved design, winner-highlighted) — `pages/Experiments.tsx` + new shared `components/DesignThumbnail.tsx`; `/variants` list now returns `config`. **Still open:** the legacy `experimentsV1` allocation/guardrails in `routes/experiments.ts` remain snippet-unconsumed — either wire them or remove that endpoint (the variants path is the real A/B). Flag `ff_experiments_v1` (off). |
