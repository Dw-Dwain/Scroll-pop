...
>
> **🔒 June 11, 2026 (latest) — DB-level RLS is LIVE on Fly + designer fixes.** **C-1 row-level security is now enabled and verified in production** (dedicated `scrollpop_tenant` role, session-mode `DB_TENANT_URL`, `DB_RLS_ENFORCED=true`). Proven on real data: a tenant role sees 14 of 22 campaigns, 0 cross-tenant leak, fail-closed with no tenant set; `events` (compressed hypertable) correctly stays app-layer-only. Plain-terms guide + exact steps: [`infra/db/RLS-ENABLEMENT-RUNBOOK.md`](infra/db/RLS-ENABLEMENT-RUNBOOK.md) and MASTER "June 11 (cont. 2)". Same session (code on working tree, **not yet committed**): transactional campaign-save (#4, new `PUT /campaigns/:id/canvas`), real visual A/B previews on Experiments (#3 → NQ-2), template-category **dropdown** (#6). See "🔧 June 11 (cont. 2)" section below.
>
