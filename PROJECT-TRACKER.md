...
> - **C-1 (Critical) — DB-level tenant backstop ATTEMPTED → REVERTED (`d102833`), now DEFERRED.** The runtime FORCE-RLS design (per-request `app.current_tenant` GUC, two-pool + `db` proxy) **crash-looped on Fly's pgbouncer pooler** (rejects the `options` startup param; drops session GUCs in transaction-pool mode) → boot threw → restart loop. Reverted to inert (`DB_RLS_ENFORCED` can't crash the app). Proper re-attempt needs a **session-mode/`DIRECT_DATABASE_URL` connection** for the tenant pool + `BYPASSRLS`/`NOBYPASSRLS` roles on `scrollpop-db`. Artifacts kept: migration `0014_real_rls`, `ensure-rls.ts`, design at `72d606f`. **App-layer `tenantId` filtering (verified consistent) remains the active isolation control.**
...
> Security: the 19 shipped fixes harden auth/secrets/SSRF/CORS/CSP. The one remaining gap — a DB-level isolation backstop (C-1) — is **deferred** (crash-looped on the pooled connection; needs a direct connection + RLS roles). App-layer tenant filtering remains the active, audited control, so this is a defence-in-depth gap, not an active vulnerability.
