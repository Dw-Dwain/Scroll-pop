...
> Sourced from `CTO-AUDIT.md` (June 4, 2026). Last reconciled: **June 18, 2026**.
> **Priority:** P0 = launch blocker · P1 = high · P2 = medium · P3 = low
> **Status:** ⬜ Todo · 🔄 In progress · ✅ Done · ❌ Won't build
>
> **🔄 June 18, 2026 (cont., latest) — Affiliate PR/disclosure label on every popup + core snippet gate raised 12 KB → 13 KB.** All popups (incl. existing campaigns, render-time, no migration) now render a compliance disclosure — a "広告 · PR" tag + one-line note (default "アフィリエイト広告を含みます", overridable via `design.disclosure`, escaped) in a non-overlapping bottom strip. Core `p.js` gzip gate raised **12288 → 13312 B (13 KB)** in CI budgets + snippet `size-check` + e2e + all docs (still ~11× under Google's 150 KB guidance; core ≈ 12.3 KB). Spin-wheel popups (separate render path) = follow-up. Closes the C-series in-popup disclosure to-do (price/"as of date" disclaimer still pending).
> **🔄 June 18, 2026 — Analytics UX + latent meta-bug fix + single-click X-close ad redirect (4 PRs merged to main, live).** **#99** per-campaign **traffic trend chart** + **24h** toggle on Campaign Detail (`/analytics/campaigns/:id` returns a dense zero-filled `series` + `granularity` in `meta`, accepts `?hours=`). **#100** root-caused the **empty trend chart + `0.00%` CTR** on Campaign Detail to the dashboard data provider's `custom()` **dropping `meta`** (chart `series` + bounded CTR live there) — now preserved + regression test; same PR made **24h uniform** across Dashboard/Analytics via a shared `?hours`/`?days` window on overview/campaigns/breakdown/revenue/funnel/intelligence + a windowed `series` on `/analytics/daily` (also fixed the Analytics trend chart ignoring 7/30/90). **#101** Dashboard **Campaign performance board** — per-campaign window totals (impr/views/clicks/CTR/conv) + a **daily-clicks sparkline**, sortable, row-click expands an inline daily chart; replaced the Top-campaigns leaderboard; new `GET /analytics/campaigns/daily` returns every campaign's dense series in one query (customer ask: "clicks per campaign on a daily basis, in one place"). **#102** snippet **single-click X-close ad redirect** (owner-requested) — for `adClose` campaigns the ✕ opens the affiliate link in a new tab **and** closes the popup in one click (was two-step); `dismiss(viaAd)` exempts ad-closes from the `popup_close` beacon + rage guard so re-shows/auto-reopen stay alive; verified live; core `p.js` 12141 B ≤ 12288 gate. All four: typecheck/lint clean; **256 API + 7 dashboard + 31 snippet tests pass**. *(`adClose` is a supported, documented feature — not a compliance item; the compliance queue is unchanged below.)* **Doc fixes:** prod DB = **Fly Postgres Tokyo (`scrollpop-db-nrt`)**, not Neon (`CONTRIBUTING.md`); snippet gate = **12 KB** gzipped (corrected the stale 10 KB refs).
>
> **🔄 June 17, 2026 — Tokyo data-residency migration LIVE + full security/compliance audit remediated (6 PRs merged to main).** **Infra:** first-party DB + API + Redis moved `ord`→**`nrt` (Tokyo)** for APPI residency (logical restore into a fresh `scrollpop-db-nrt` TimescaleDB cluster — faithful, events row-parity 36,083; ~10–15 min downtime at the 04:00 JST trough; #84 `primary_region=nrt`; Redis → Tokyo `ap-northeast-1` Upstash on API+Worker). Runbook `docs/nrt-migration-plan.md`; RLS re-verified (42 own / 0 leak). PostHog/Sentry confirmed (US/EU, IP-minimized). **Security audit (8-agent + verification):** every actionable finding fixed/merged — **#87** spin-wheel + consent stored-XSS (S1/S7); **#88** `/e` ingest — origin-gate **all** event types, `email_capture` flood cap, fail-closed `isFromWorker`, metadata bounds (S5/S6/S9/S13); **#89** tenant plan-escalation/IDOR + token-crypto fail-closed + targeting regex (S2/S10/S12); **#90** untrack committed GitHub PAT fragments + `scratch/` (S8 — ⚠️ **OWNER MUST REVOKE the 3 PATs, still in git history**); **#91** Shopify `orders/paid` idempotency + webhook replay window (S4/S11). S3 verified FALSE POSITIVE (OAuth HMAC covers `state`); S14 deferred (Fly-specific `trustProxy`). **Bugs #92:** `trigger_blocked` once-per-load (diagnose funnel) + Analytics precedence (B1/B4); B2/B3 deferred. **Sites/analytics:** verify-snippet now follows redirects (#85 — Shopify custom-domain stores verify, no more 422); fixed 2 sites stored on `.myshopify.com` backend → storefront (`stellaseed-onlinestore.jp`, `sakananet.com`) so their impressions/conversions count under the origin gate; CTR unified across views (#86). All typecheck-clean, **256 API tests pass**. **Open:** compliance **C1–C4** (EU opt-in consent, privacy-policy → Fly/Tokyo, DPA/SCC, retention — C1/C4 to build, C2/C3 to draft for legal); **revoke 3 GitHub PATs**.
>
> **🔄 June 15, 2026 — Per-site affiliate links built** (branch `feat/affiliate-links-per-site`, not yet merged). Operators save multiple **named affiliate links per site** (`sites.affiliate_links` JSONB), managed in **Settings → Affiliate links** — a new server-backed editor that **replaced** the old localStorage-only "Affiliate Networks" card (closes the affiliate half of **NQ-3**). The campaign designer surfaces them as a **picker** that auto-fills the X-close ad link + CTA/affiliate-button `href`. Layers: shared `AffiliateLinkSchema` + type; DB column via `schema.ts` + idempotent `ensure-affiliate-links.ts`, **SCHEMA_VERSION 19→20**; `PATCH /sites/:id` validation (≤100, http(s)). **No snippet change** (plain `href`s via `safeHref`). Typecheck (shared/api/dashboard) + lint clean; schema accept/reject runtime-verified. Deploy = API redeploy on Fly (ensure-* runs the `ADD COLUMN` on boot).
>
> **🔒 June 11, 2026 — DB-level RLS is LIVE on Fly + designer fixes.** **C-1 row-level security is now enabled and verified in production** (dedicated `scrollpop_tenant` role, session-mode `DB_TENANT_URL`, `DB_RLS_ENFORCED=true`). Proven on real data: a tenant role sees 14 of 22 campaigns, 0 cross-tenant leak, fail-closed with no tenant set; `events` (compressed hypertable) correctly stays app-layer-only. Plain-terms guide + exact steps: [`infra/db/RLS-ENABLEMENT-RUNBOOK.md`](infra/db/RLS-ENABLEMENT-RUNBOOK.md) and MASTER "June 11 (cont. 2)". Same session, shipped: transactional campaign-save (#4, new `PUT /campaigns/:id/canvas`, committed `8a452af`, **pushed to main/live**), real visual A/B previews on Experiments (#3 → NQ-2), template-category **dropdown** (#6), white-circle X-close button (designer + snippet) + X-only popups (removed backdrop tap-to-dismiss; snippet budget 10300→10400). Then (branch `journeys-node-builder`): **Journeys node builder** (#2 → NQ-6, `PUT /journeys/:id/link` drives the live chain runtime) and **creative templates** (#5: replaced the single generic vertical template with **7 dedicated 330×560 templates, one per creative JPG**; the creatives thumbnail picker is intact in the designer's right panel for image elements). See "🔧 June 11 (cont. 2)" section below.
>
...
| NQ-2 | ✅ **done (Jun 22)** | Experiments honesty + visual A/B + bandit | The Experiments **page** reads the **real variants** system (`/variants` + `/variants/results`) with real telemetry. Visual A/B preview cards (winner-highlighted) shipped Jun 11. **Jun 22: resolved the duplication — DELETED the snippet-unconsumed `routes/experiments.ts` (`experimentsV1`) and standardized on the variants system.** Also hardened A/B: results/significance now use bounded **unique-visitor** counts and are **objective-aware** (CTR vs conversion); added a **Thompson-sampling bandit** scheduler (`db/ab-bandit.ts`, advisory-locked, opt-in per campaign via `campaigns.ab_config`, ~2.5h cadence, writes `variants.weight` + purges edge cache) with pure math in `lib/bandit.ts` (tested); added **pause/resume** + auto-optimize toggle + objective selector (`PUT /variants/experiment`); promote now disables the bandit. Flag `ff_experiments_v1` (dashboard nav gate) unchanged. |
...
| NQ-6 | ✅ **node builder shipped (Jun 11)** | Journeys runtime (→ FU-7) | **Diagnose** already used real `trigger_blocked` telemetry (the old "fabricated 40/35/25%" note was stale). **Jun 11: built the node-based journey builder.** New `PUT /journeys/:id/link` writes the chain edge into `design.config.uiTriggers.sequence*` — exactly what the snippet's existing `journey.js` chunk consumes, so chains run **live with no snippet change**. Server-enforced guardrails: same-site only, no self-link. `GET /journeys` now returns `siteName` + each node's `sequence`. The Journeys **page** is a real per-site flow graph: campaign **nodes** with thumbnails, **edges** (advance-on dismiss/convert + delay), inline link editor (set/clear next step), runtime guardrail banner (max 2 chained, ≥5s gap, no repeats), and per-node live diagnostics. 5 new API tests (86→91). **Remaining:** (a) remove the now-redundant sequence control from the campaign designer (`SidebarLeft.tsx:~1611`); (b) branches/multi-next (runtime is single-next today); (c) Playwright e2e. Flag `ff_journeys_ui` (off). |
...
> - **C-1 (Critical) — DB-level tenant backstop: crash-looped → root-caused → REBUILT + validated, ready to enable.** First attempt (reverted `d102833`) was broken: tenant pool connected as **superuser** (bypasses RLS → enforced nothing) + a `-c app.bypass_rls=on` startup param pgbouncer rejected (→ crash-loop). Reproduced both vs a real PG17+TimescaleDB container and rebuilt: system pool bypasses by **role attribute** (no GUC/param), tenant pool = dedicated **`scrollpop_tenant` NOBYPASSRLS** role on a session-mode conn + per-request `app.current_tenant` GUC, tenant-predicate-only policies. Wiring uses callback-style `onRequest` `AsyncLocalStorage.run` (enterWith doesn't propagate across Fastify hooks). **Validated: isolation holds incl. `events` hypertable + GUC-escape blocked + zero leak across 20 concurrent interleaved requests.** FAIL-SAFE (degrades to app-layer-only, never crashes). Cross-tenant routes (`admin`/`team`/`me`/`emitNotification`) use `systemDb`. **Gated by `DB_RLS_ENFORCED`+`DB_TENANT_URL`; enable after one-time `scrollpop_tenant` role + session-mode `DB_TENANT_URL`, staging-tested (MASTER §RLS).** Files: `db/client.ts`, `db/ensure-rls.ts`, migration `0014_real_rls`.
...
> Security: the 19 shipped fixes harden auth/secrets/SSRF/CORS/CSP. C-1 (DB-level isolation backstop) is now **built, validated against a real DB, and ready to enable** behind `DB_RLS_ENFORCED`+`DB_TENANT_URL` (one-time role setup + staging test pending). App-layer tenant filtering remains the active, audited control until it's switched on — so the DB backstop is a defence-in-depth add, never an active vulnerability.
...
|---|---|---|---|---|
| BUG-5 | 🔄 Fixed (working tree, Jun 12) | High | **Journeys node editor rendered ~10–20px wide** (canvas collapsed to a sliver, whole editor centered). The Layout content wrapper applied `className="page-enter"` on every non-fullscreen route; the global `.page-enter > div` rule sets `max-width:1500px;margin-left/right:auto`, and auto side-margins on a flex child override `align-items:stretch` → the editor's flex root shrank to content width and centered. Profile/Settings escaped it only because their roots are `<section>` (the rule is `> div`). | Excluded `isSplitPage` (Journeys/Profile/Settings — full-bleed flex layouts) from `page-enter` in `Layout.tsx`. Verified in new auth-free harness (`apps/dashboard/journeys-harness.html` + `src/journeys-harness.tsx`): editor canvas 47px → 592px, root full viewport width. |
| BUG-4 | 🔄 Fixed (working tree, Jun 12) | High | **X-close two-step (1st click opens affiliate ad → 2nd click closes) worked on old campaigns, not new ones.** New creative-template campaigns keep the affiliate link on the **image** element (`cr-img.href`), not the close button; the snippet derived the ad-close URL only from `closeEl.href`/affiliate-slot URLs, so it was empty → instant close. The designer preview's `getStepAffiliate` falls back to the step's link, so the simulation worked but production didn't. | Snippet ad-close derivation now falls back to the step's primary affiliate href (image/button `href`), mirroring `getStepAffiliate` (skips `YOUR_`/`REPLACE-` placeholders). `main.ts` ~1227. |
| BUG-3 | 🔄 Fixed (working tree, Jun 12) | High | **Image/X affiliate links resolved to the host page** instead of the configured URL. Operators paste scheme-less links (`www.shop.com/x`); `safeHref` did `new URL()` which throws on those → returned `'#'`, so `<a href="#">` navigates to the host page (the popup's own blog), and the ad-close got no URL → instant close. Same scheme-less class as the `url_exact` bug (`438a651`). | `safeHref` normalizes scheme-less / protocol-relative links to `https://` (rejects bare relative paths like `/blog`, still blocks `javascript:`/`data:`). +6 sanitizer tests (30 pass). Batched into the same snippet rebuild as BUG-4 + the staged exit-intent desktop-only guard; `p.txt` resynced, gzip 11092/11264B. |
