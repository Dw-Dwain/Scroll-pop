...
The dashboard hands customers a **pre-built zip** served from R2 (bucket `scrollpop-assets`,
object `scrollpop-wp.zip`; the public URL + `?v=` cache token live in `apps/dashboard/src/pages/Sites.tsx`).
The Worker does not serve it — it's a direct R2 object.
...
- **Verify the LIVE object via RAW BYTES, not `zipfile.namelist()`** — namelist cosmetically shows `/` even
  ...
  Download `…/scrollpop-wp.zip?v=<unique>` and assert `data.count(b'scrollpop\x5c') == 0` (backslashes) and
