...
└── infra/
    ...
    └── db/           # Migration files, RLS policies, seed data
```
...
| Database | PostgreSQL 17 (Fly.io managed Postgres, postgres-flex) | RLS on every tenant-scoped table |
| Analytics DB | TimescaleDB (same Fly Postgres instance, hypertable) | Migrate to ClickHouse at >50M events/month |
...
## Build Artifacts — Snippet Bundle Sync
The Cloudflare Worker serves a **pre-built copy** of the snippet at `apps/worker/src/p.txt`.
It is generated by `pnpm --filter snippet build` (which builds `dist/p.js` AND syncs `p.txt`).
- **Whenever you edit `packages/snippet/src/**`, you MUST rebuild and commit `apps/worker/src/p.txt`
  in the same commit.** CI (`snippet-size-check`) rebuilds the snippet and fails if the committed
  `p.txt` differs from a fresh build.
- A versioned pre-commit hook (`.githooks/pre-commit`, enabled via `core.hooksPath` by the root
  `prepare` script on `pnpm install`) does this automatically. If you commit with `--no-verify`
  or in an environment where the hook didn't activate, run the rebuild manually.
- `p.txt` is a generated artifact — never edit it by hand.

## Build Artifacts — WordPress Plugin Zip (R2)
The dashboard hands customers a **pre-built zip** stored in R2 (bucket `scrollpop-assets`,
object `scrollpop-wp.zip`; the download URL + `?v=` cache token live in `apps/dashboard/src/pages/Sites.tsx`).
The **Worker serves it** from that R2 bucket at **`https://cdn.scrollpop.online/scrollpop-wp.zip`**
(route in `apps/worker/src/index.ts`) — over our own production domain, NOT the rate-limited `*.r2.dev`
public bucket URL (P3-3). It's still the same R2 object; only the delivery path is the Worker.
- **The zip MUST be the `build-zip.py` output, which writes forward-slash (`/`) paths and verifies them.**
  NEVER hand-zip the plugin (Windows Explorer "Send to → Compressed folder", `Compress-Archive`, etc.) —
  those write **backslash** separators (`scrollpop\scrollpop.php`). WordPress runs on Linux, treats `\`
  as a literal filename char, never finds the `scrollpop/` folder, and activation dies with **"Plugin
  file does not exist."** (installs as `scrollpop-wp-N/scrollpop/scrollpop.php`). This has caused repeated
  customer outages.
- **Whenever you edit `packages/wp-plugin/scrollpop/**`, you MUST, in the same change:**
  1. Rebuild: `pnpm --filter @scrollpop/wp-plugin package` (runs `build-zip.py`).
  2. Bump `Version:` + `SCROLLPOP_VERSION` in `scrollpop.php` **and** the matching `PLUGIN_VERSION` cache
     token in `apps/dashboard/src/pages/Sites.tsx` — the `?v=` query is what evicts the stale (possibly
     broken) cached object from R2/edge. Keep them in sync.
  3. Re-upload to R2 (from `apps/worker`, after `wrangler login`):
     `WRANGLER_SEND_METRICS=false pnpm exec wrangler r2 object put scrollpop-assets/scrollpop-wp.zip --file ../../packages/wp-plugin/dist/scrollpop-wp.zip --content-type application/zip --remote --force`
     (the env var stops wrangler's telemetry prompt from hanging non-interactive shells; or upload via the
     Cloudflare dashboard → R2 → `scrollpop-assets`, overwriting the object).
- **Verify the LIVE object via RAW BYTES, not `zipfile.namelist()`** — namelist cosmetically shows `/` even
  when the stored bytes are `\`, and a `/`→`\` swap keeps the byte size identical (only the SHA changes).
  Download `https://cdn.scrollpop.online/scrollpop-wp.zip?v=<unique>` and assert `data.count(b'scrollpop\x5c') == 0` (backslashes) and
  `data.count(b'scrollpop\x2f') > 0` (slashes); the live SHA should equal the local `dist/scrollpop-wp.zip`.
- `packages/wp-plugin/dist/scrollpop-wp.zip` is a generated artifact (gitignored) — never edit by hand.

## Environment Variables

### apps/api
```
DATABASE_URL=              # Fly Postgres connection string (pooler)
